...
class Container:
	...
	def add(self, obj):
		if self.size > 0 and len(self.inventory) >= self.size:
			return False
...
class Fighter:
	...
	def attack(self, target):
		...
		if damage > 0:
			...
			# take minimal damage
			message(self.owner.name.capitalize() + ' attacks ' + target.name +', but it is not very effective!', libtcod.yellow)
...
class Equipment:
	...
	def unequip(self, wearer):
		...
		self.is_equipped = False
		message(wearer.name.capitalize() + ' unequipped ' + self.owner.name + ' from ' + self.slot + '.', libtcod.light_yellow)
...
def place_objects(room):
	...
	monster_chances = {}
	monster_chances['zombie'] = 60 # zombies always show up, even if all other monsters have 0 chance
	monster_chances['orc'] = from_dungeon_level([[50,1],[60,2],[70,3]])
	monster_chances['troll'] = from_dungeon_level([[15,3],[30,5],[60,7]])
	monster_chances['ogre'] = from_dungeon_level([[15,7],[30,10],[40,13]])
	monster_chances['dragon'] = from_dungeon_level([[10,10],[20,13],[25,15]])
	monster_chances['cthulhu'] = from_dungeon_level([[10,13],[15,15]])
	...
	item_chances['small_heal'] = from_dungeon_level([[35,1],[10,6]]) # healing potion always shows up
	item_chances['lightning'] = from_dungeon_level([[25,4]])
	item_chances['fireball'] = from_dungeon_level([[25,6]])
	item_chances['confuse'] = from_dungeon_level([[10,2],[15,6]])
	item_chances['big_heal'] = from_dungeon_level([[35,6]]) 
	...
	for i in range(num_monsters):
		...
		if choice == 'zombie':
			fighter_component = Fighter(hp=10, defense=0, power=2, xp=15, death_function=monster_death, attack_effect_function=zombie_bite)
			ai_component = BasicMonster()
			monster = Object(x,y,'z','zombie', libtcod.chartreuse, blocks=True, fighter=fighter_component, ai=ai_component, controller=controller)
		elif choice == 'orc':
			orc_bag = Container(2)
			...
			objects.append(item)
			fighter_component = Fighter(hp=20, defense=1, power=4, xp=25, death_function=monster_death, attack_effect_function=orc_berserk)
			ai_component = BasicMonster()
			monster = Object(x,y,'o','orc', libtcod.desaturated_green, blocks=True, fighter=fighter_component, ai=ai_component, controller=controller, container=orc_bag)
			item.item.pick_up(monster)
		elif choice == 'troll':
			fighter_component = Fighter(hp=30, defense=2, power=8, xp=75, death_function=monster_death)
			ai_component = BasicMonster()
			monster = Object(x,y,'T','troll', libtcod.desaturated_green, blocks=True, fighter=fighter_component, ai=ai_component, controller=controller)
		elif choice == 'ogre':
			fighter_component = Fighter(hp=40, defense=2, power=10, xp=100, death_function=monster_death)
			ai_component = BasicMonster()
			monster = Object(x,y,'O','ogre', libtcod.desaturated_red, blocks=True, fighter=fighter_component, ai=ai_component, controller=controller)
		elif choice == 'dragon':
			fighter_component = Fighter(hp=60, defense=3, power=15, xp=150, death_function=monster_death)
			ai_component = BasicMonster()
			monster = Object(x,y,'D','dragon', libtcod.red, blocks=True, fighter=fighter_component, ai=ai_component, controller=controller)
		elif choice == 'cthulhu':
			fighter_component = Fighter(hp=100, defense=4, power=20, xp=200, death_function=cthulhu_death)
			ai_component = BasicMonster()
			monster = Object(x,y,'C', 'cthulhu', libtcod.brass, blocks=True, fighter=fighter_component, ai=ai_component, controller=controller)
...
def make_room(node=None, userData=None):
...
def render_all():
	...
	render_bar(1,2,BAR_WIDTH, 'XP', player.fighter.xp, LEVEL_UP_BASE + player.level * LEVEL_UP_FACTOR, libtcod.light_violet, libtcod.darker_violet)
...
def cthulhu_death(monster):
	# GAME OVER YOU WIN!!
	global game_state
	message('The horror of the depths, ' + monster.name.capitalize()+ ', has been slain! You have saved the world from destruction!',libtcod.green)
	monster.char = '%'
	monster.blocks = False
	monster.fighter = None
	monster.ai = None
...
def check_level_up():
	...
	if player.fighter.xp >= level_up_xp:
		...
		choice = None
		while choice is None: # keep asking untill a choice is made
			choice = menu('Level up! Choose a stat to raise!\n',
				['Constitution (+ 20 hp, from ' + str(player.fighter.max_hp) + ' to ' + str(player.fighter.max_hp+20) + ')',
				'Strength (+ 1 attack, from ' + str(player.fighter.power) + ' to ' + str(player.fighter.power + 1) + ')',
				'Agility (+ 1 defense, from ' + str(player.fighter.defense) + ' to ' + str(player.fighter.defense + 1) + ')'], LEVEL_SCREEN_WIDTH)
			if choice == 0:
...
def inventory_menu(header):
	...
	if index is None or len(player.container.inventory) == 0: return None
...
def save_game():
	...
	file['player_index'] = objects.index(player) # index of player in objects list, cant save player object directly because already in objects
...
def load_game():
	...
	global map, objects, player, game_msgs, game_state, stairs, dungeon_lvl
	file = shelve.open('savegame', 'r')
	map = file['map']
	dungeon_lvl = file['dungeon_lvl']
